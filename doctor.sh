...
if command -v git-lfs >/dev/null 2>&1; then
    ...
    if [ -n "$_git_hooks_dir" ] && [ -f "${_git_hooks_dir}/pre-push" ]; then
        ...
        _fail "git-lfs hooks not registered  (fix: ~/dev-bootstrap/run.sh setup_git_lfs)"
    fi
fi
...
if [ -f "${HOME}/.gnupg/gpg-agent.conf" ]; then
    ...
    if grep -q "default-cache-ttl-ssh" "${HOME}/.gnupg/gpg-agent.conf" 2>/dev/null; then
        ...
        _warn "gpg-agent SSH cache TTL not set  (fix: ~/dev-bootstrap/run.sh setup_gpg_agent_conf)"
    fi
    ...
    if [ -n "$_pinentry_path" ] && [ -x "$_pinentry_path" ]; then
        ...
        _warn "pinentry  not found at configured path (${_pinentry_path:-none})  (fix: ~/dev-bootstrap/run.sh setup_gpg_agent_conf)"
    fi
else
...
if command -v bats >/dev/null 2>&1; then
    ...
    _warn "bats  not installed  (only needed to run this repo's own tests - fix: ~/dev-bootstrap/run.sh setup_bats)"
fi
...
if command -v shellcheck >/dev/null 2>&1; then
    ...
    _warn "shellcheck  not installed  (only needed to lint this repo's own scripts - fix: ~/dev-bootstrap/run.sh setup_shellcheck)"
fi
