...
finish() {
    ...
    echo ""
    note "Download all four fonts from the repo (click each link to download):"
    note "https://github.com/DevTruce/dev-bootstrap/tree/main/fonts"
    echo ""
    case "$OS" in
        macos)
    ...
    note "  bash ${DOTFILES_DIR}/doctor.sh"
    echo ""
    printf "  ${BOLD_CYAN}════════════════════════════════════════════════════${RESET}\n"
    echo ""
}
