...
│   └── workflows/               # lint.yml (shellcheck + zsh -n), test.yml (./contributing/test.sh), release-please.yml - see Contributing
...
├── CHANGELOG.md                 # auto-generated by release-please - see Contributing
└── LICENSE                      # MIT
```

`.github/` and `contributing/` are the two contributor-only pieces - both are safe to delete
wholesale if you're only using this repo to set up a machine and never plan to submit changes
back. Deleting `.github/` just turns off CI/release automation; deleting `contributing/` removes
the test suite, linter config, and local CI runner. Neither is read by `install.sh`, `run.sh`, or
`doctor.sh`.
...
```bash
./contributing/test.sh
```

- Runs every `*.bats` file under `contributing/tests/` via the `bats` test runner (installed above).
- `contributing/tests/` follows a naming convention mirroring `scripts/` and the root-level `.sh`
  files where a test exists - e.g. `contributing/tests/scripts/helpers.bats` tests
  `scripts/helpers.sh`, `contributing/tests/run.bats` tests `run.sh`. Coverage isn't exhaustive -
  not every script has a `.bats` file yet. **These `.bats` files are test files, not runners** -
  `contributing/tests/run.bats` doesn't run anything, it just contains unit tests for `run.sh`'s
  functions. `./contributing/test.sh` is always the command that runs the suite.
